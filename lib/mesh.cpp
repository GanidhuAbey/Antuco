...
Mesh::Mesh(std::vector<Vertex> new_vertices, std::vector<uint32_t> new_indices, MaterialsObject mat_obj, std::string texture_path) {
    vertices = new_vertices;
    indices = new_indices;
    mat_data = mat_obj;

    if (mat_data.texture_opacity.x) {
        LOG(texture_path.c_str());
