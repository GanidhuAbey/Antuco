...
Light::Light(glm::vec3 light_pos, glm::vec3 light_target, glm::vec3 light_color, glm::vec3 up) {
	...
	world_to_light = glm::lookAt(glm::vec3(position), glm::vec3(target), up);
}

void Light::update(glm::vec3 translation) {
	position = glm::vec4(translation, 0.0);
}
