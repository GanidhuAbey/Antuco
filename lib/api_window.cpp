...
WindowImpl::WindowImpl(int w, int h, const char *title) {
  // for now we can create a GLFW window but if we ever want tp use a different
  // window API, the user won't have to relearn our API
  if (!glfwInit())
...
/// </summary>
/// <param name="windowEvent"> the window status that the user is querying for
/// </param> <returns> the state of the window event that the user is querying
/// for </returns>
bool WindowImpl::check_window_status(WindowStatus windowEvent) {
  glfwPollEvents();
  switch (windowEvent) {
  case WindowStatus::CLOSE_REQUEST:
    return glfwWindowShouldClose(apiWindow);
  }

  printf("[ERROR] - The WindowStatus code is not queried by "
         "check_window_status() \n");
...
/// </summary>
/// <param name="x_pos"> the x position of the mouse (data in this variable will
/// be overwritten) </param> <param name="y_pos"> the y position of the mouse
/// (data in this variable will be overwritten) </param>
...
/// <returns> status of the window input that the user is querying for
/// </returns>
...
void WindowImpl::create_vulkan_surface(VkInstance instance,
                                       VkSurfaceKHR *surface) {
  VkResult surface_result =
      glfwCreateWindowSurface(instance, apiWindow, nullptr, surface);
  if (surface_result != VK_SUCCESS) {
    printf("[ERROR CODE: %d] - could not create vulkan surface on window",
           surface_result);
...
/// closes window
/// </summary>
void WindowImpl::close() { glfwWindowShouldClose(apiWindow); }
